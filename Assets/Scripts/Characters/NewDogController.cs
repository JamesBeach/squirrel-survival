...
public class NewDogController : VersionedMonoBehaviour, ICharacterController {

    public float leashRange;
    private AudioSource bark;
	//my variables 
	private bool gameStarted = false;
	private bool squirrelAlive = true;
	...
	protected virtual void Start () {
        // register with LevelController, if it exists
        {
            LevelController lvlctrl = FindObjectOfType<LevelController>();
            if (lvlctrl != null) lvlctrl.RegisterCharacter(this);
            else
            {
                Debug.Log(this.GetType().Name + " did not find a LevelController!");
            }
        }
	...
	public float TrySearchPath () {
		if (Time.time - lastRepath >= repathRate && canSearchAgain && canSearch && target != null && gameStarted) {
			SearchPath();
			return repathRate;
		} else {
	...
	public virtual void ForceSearchPath () {
		...
		if(SquirrelController.inTree || Vector2.Distance (target.transform.position, transform.position) > leashRange){
